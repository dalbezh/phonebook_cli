...
В [main.py](./main.py) находятся основные вызовы в блоке `__name__ == '__main__'`. В main.py создаются все необходимые 
...
в отдельно файл, чтобы не перегружать основную логику работы со справочником. В конце utils.py инициализируется основной 
класс, который используется в работе как utils.py, так и main.py.\
